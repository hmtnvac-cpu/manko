...
async function enrichManko(movieId,pageTitle,pageMeta){
 const [detail,actorsApi]=await Promise.all([
  getJson(`${HEAL}/swx/movie/detail/${encodeURIComponent(movieId)}`),
  getJson(`${HEAL}/swx/movie/actors/${encodeURIComponent(movieId)}?size=20&page=1`)
 ]);
...
chrome.runtime.onMessage.addListener((msg,sender,sendResponse)=>{(async()=>{
 if(msg.type==='START_QUEUE'){
  const s=await getState();const done=new Set((s.results||[]).map(x=>x.movieUrl));const busy=new Set([...(s.queue||[]),s.current?.movieUrl].filter(Boolean));
  const incoming=[...new Set((msg.urls||[]).map(x=>String(x).trim()).filter(isManko))];const added=incoming.filter(u=>!done.has(u)&&!busy.has(u));
  s.queue.push(...added);s.totalDiscovered=Math.max(Number(s.totalDiscovered||0),incoming.length);await setState(s);processNext();sendResponse({ok:true,added:added.length,alreadyDone:incoming.length-added.length,total:incoming.length});return;
 }
 if(msg.type==='MANKO_PLAYER_FOUND'){
  const s=await getState();if(!s.current)return;s.current.movieId=msg.movieId||((msg.movieUrl.match(/\/movie-info\/([^/?#]+)/)||[])[1]||'');s.current.title=msg.title||'';s.current.poster=msg.poster||'';s.current.playerUrls=[...new Set((msg.playerUrls||[msg.playerUrl]).filter(Boolean))];s.current.playerIndex=0;s.current.streams=[];
  s.current.metadata=await enrichManko(s.current.movieId,msg.title,msg.metadata||{});s.current.titleVi=s.current.metadata.titleVi||cleanPageTitle(msg.title);await setState(s);
  if(!s.current.playerUrls.length){sendResponse({ok:false});return finishCurrent(false,{error:'No javplayer embed'})}await openNextPlayer();sendResponse({ok:true,players:s.current.playerUrls.length});return;
 }
 ...
 if(msg.type==='CLEAR_ERRORS'){const s=await getState();s.errors=[];await setState(s);sendResponse({ok:true});return}
 })();return true});
