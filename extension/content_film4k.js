(() => {
  ...
  function add(url,kind='resource',extra={}){
    ...
    seen.add(uniqueKey);resources.push({url:u,kind,...extra});if(resources.length>700)resources.splice(0,resources.length-700);
  }
  function snapshot(){
  ...
  const meta=()=>({title:document.querySelector('meta[property="og:title"]')?.content||document.title||'',poster:document.querySelector('meta[property="og:image"]')?.content||document.querySelector('meta[name="twitter:image"]')?.content||'',description:document.querySelector('meta[name="description"]')?.content||document.querySelector('meta[property="og:description"]')?.content||'',canonical:document.querySelector('link[rel="canonical"]')?.href||location.href,frameUrl:location.href,topFrame:window===top,collectorVersion:'4.2.2'});
  let sending=false,pending=false;
  async function report(){
    if(sending){pending=true;return}
    sending=true;pending=false;snapshot();
    try{await chrome.runtime.sendMessage({type:'FILM4K_PROBE',pageUrl:top===window?location.href:(document.referrer||location.href),meta:meta(),resources:resources.slice(-700)})}catch{}
  ...
  function start(){if(!document.documentElement)return setTimeout(start,50);obs.observe(document.documentElement,{subtree:true,childList:true,attributes:true,attributeFilter:['src','href']});snapshot();report();let n=0;const t=setInterval(()=>{snapshot();report();if(++n>=90){clearInterval(t);obs.disconnect()}},1000)}
  start();
})();